...
namespace BI.SGP.BLL.Models
{
    ...
    public class RFQManager
    {
        ...
        public static bool CheckData(HttpRequestBase Request, ref SystemMessages message)
        {
            ...
            FieldInfoCollecton allFields = FieldCategory.GetAllFields(FieldCategory.Category_TYPE_SGP);
            
            foreach (string key in Request.Form.Keys)
            {
                CheckData(key, Request.Unvalidated.Form[key], allFields, ref message);
            }
            return true;
        }
        ...
        public static void UpdateSpecialFields(int rfqid)
        {
            ...
            if (rfqdetails != null)
            {
                ...
                string strsqlforPricing = string.Format(@"update SGP_RFQPricing set 
                                               ...
                                               , rfqdetail.MP7, rfqdetail.MP8, rfqdetail.MP9, rfqdetail.MP10,rfqdetail.TargetASPL,rfqdetail.MinASPL);
...
", newExtNumber);
        ...
        private static int CreateNewRFQAllTablesForVVIMultek(
         string newNumber, string newExtNumber, string building, string rev, string uid)
        {
            string strSql = string.Format("INSERT INTO SGP_RFQ(Number,ExtNumber,Building,InternalRevisionNumber, EmployeeID) VALUES('{0}','{1}','{2}','{3}','{4}');SELECT @@IDENTITY"
                , newNumber, newExtNumber, building, rev, uid);
...
//            DbHelperSQL.ExecuteSql(strSql);
        ...
        private static int CreateNewRFQIDBuildingChangeForVVIMultek(int oldRFQId, RFQDetail rfqDetail, string newBuilding, string newRev, string uid, out string newInternalNumber)
        {
            newInternalNumber = rfqDetail.Number;
            newRev = newRev.PadLeft(2, '0');
            string oldExtNumber = rfqDetail.ExtNumber;
            ...
            //创建新的RFQ数据
            int rfqId = CreateNewRFQAllTablesForVVIMultek(newInternalNumber, oldExtNumber, newBuilding, newRev, uid);
...
WHERE EntityId={1} ORDER BY ID ASC", rfqIdTo, rfqIdFrom);
        ...
        public static bool ReQuote(ref int rfqId, Dictionary<string, Dictionary<string, string>> dicAllTable, FieldInfoCollecton fields)
        {
            ...
            string sql = string.Format("update SGP_RFQGeneral set RFQDateIn=GETDATE(),LastestRFQDateIn=GETDATE(),RFQDateOut=null,QuoteDateIn=null,QuoteDateOut=null,PriceDateOut=null where RFQID={0}", rfqId);
        ...
        public static bool Update(ref int rfqId, FieldInfoCollecton fields)
        {
            ...
            if (detail != null)// 如果在有老数据存在的情况下，判断Building是否有修改，如果有需要一条复制数据
            {
                ...
                string TechnicalQuotingOld = detail.TechnicalQuoting;
                string TechnicalQuotingNew = string.Empty;
                if (fields["TechnicalQuoting"] != null) TechnicalQuotingNew = string.Format("{0}", fields["TechnicalQuoting"].DataValue);
                if (TechnicalQuotingNew != TechnicalQuotingOld)
                {
                    UpdateLastestDateIn(rfqId);
                }
        ...
        private static void UpdateLastestDateIn(int rfqid)
        {
            string sql = string.Format("update SGP_RFQGeneral set LastestRFQDateIn=getdate() WHERE RFQID={0}", rfqid);
            DbHelperSQL.ExecuteSql(sql);
        }
                ...
                try
                {
                    ...
                    foreach (string tableName in dicAllTable.Keys)
                    {
                        ...
                        foreach (KeyValuePair<string, string> kvField in dicAllTable[tableName])
                        {
                            ...
                            if (kvField.Key == "LastestRFQDateIn") continue;
                   ...
                   // BI.SGP.BLL.Models.RFQManager.PostRFQToVVI(rfqId);
                    strSql = "Insert Into SGP_RFQHistory select *,getdate() from v_sgp where RFQID=@RFQID";
        ...
        public static NameValueCollection Save(HttpRequestBase request)
        {
            ...
            foreach (string key in request.Form.Keys)
            {
                ...
                if (field != null)
                {
                    string v = request.Unvalidated.Form[key];
                ...
                if (field != null && !String.IsNullOrEmpty(field.TableName))
                {
                    ...
                    if (dicAllTable.ContainsKey(tableName))
                    {
                        dicAllTable[tableName].Add(key, request.Unvalidated.Form[key]);
                    }
                    else
                    {
                        Dictionary<string, string> dic = new Dictionary<string, string>();
        ...
        public static int IsAttachment(string RFQID)
        {
            string strSql = "SELECT count(*) FROM SGP_FilesForVVI WHERE RelationKey = @RFQID";
            int ID = 0;
            ID = (int)DbHelperSQL.GetSingle(strSql, new SqlParameter("@RFQID", RFQID));
            return ID;
        }
        ...
        public static bool IsPendingStatus(int RFQID,int ToActID)
        {
             List<RFQDetail> rfqdetails = RFQManager.GetDetail(RFQID);
             bool ispending = true;
             if (rfqdetails.Count > 0)
             {
                 ...
                 if (rfqdetail.RFQID > 0)
                 {
                     ...
                     if ((rfqdetail.HitRateStatus == "Pending") && rfqdetail.RFQID > 0 && ToActID == 7)
                     {
                         ispending=false;
                     }
                     else
        ...
        public static NameValueCollection ReQuoteData(HttpRequestBase request)
        {
            ...
            try
            {
                ...
                if (rfqId > 0)
                {
                    string strsql = "update SGP_RFQ set ActivityID=1,StatusID=1 where ID=@ID";
                    DbHelperSQL.ExecuteSql(strsql, new SqlParameter("@ID", rfqId));
                }
