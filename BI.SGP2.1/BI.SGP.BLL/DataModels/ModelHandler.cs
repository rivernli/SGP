...
namespace BI.SGP.BLL.DataModels
{
    ...
    public class ModelHandler<T> where T : new()
    {
        ...
        public static void ColumnNameToDisplayName(DataTable dt, FieldInfoCollecton fields)
        {
            if (dt == null) return;
            if (fields == null || fields.Count <= 0) return;
            foreach (DataColumn dc in dt.Columns)
            {
                FieldInfo fi = fields.Find(t => string.Compare(t.FieldName, dc.ColumnName, true) == 0);
                if (fi != null)
                {
                    dc.ColumnName = fi.DisplayName;
                }
            }
        }
        ...
        /// <param name="dt"></param>
        /// <param name="fields"></param>
        public static void ColumnNameToFieldName(DataTable dt, FieldInfoCollecton fields)
        {
            if (dt == null) return;
            if (fields == null || fields.Count <= 0) return;
            foreach (DataColumn dc in dt.Columns)
            {
                FieldInfo fi = fields.Find(t => string.Compare(t.DisplayName, dc.ColumnName, true) == 0);
                if (fi != null)
                {
